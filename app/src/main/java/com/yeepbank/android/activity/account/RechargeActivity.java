...
public class RechargeActivity extends BaseActivity implements View.OnClickListener,AdapterView.OnItemClickListener{
    ...
    private ImageView payByCardBtn;
    ...
    private LoadDialog unboundedBankCardDialog;//未绑定银行卡时的弹窗
    private LoadDialog msgDialog;
    @Override
    protected void initView() {

        navigationBar = findViewById(R.id.navigation_bar);
        payByCardBtn = (ImageView) navigationBar.findViewById(R.id.pay_by_card_btn);
        payByCardBtn.setVisibility(View.VISIBLE);
        payByCardBtn.setOnClickListener(this);
        ...
        addBankCardBtn = (LinearLayout) findViewById(R.id.add_bank_card_btn);
        ...
        msgDialog = new LoadDialog(mContext, R.style.dialog, false, new View.OnClickListener() {
            ...
            public void onClick(View v) {
                msgDialog.dismiss();
                gotoTarget(UpdateTradePasswordActivity.class, R.anim.activity_in_from_right, R.anim.activity_out_from_left, "");
            }
        ...
        switch (v.getId()){
           ...
           /* case R.id.bank_list_layout:
            ...
            case R.id.recharge_sure_btn:
                ...
                //rechargePre();
                break;
            case R.id.pay_by_card_btn:
                gotoTarget(PayByCardForRechargeActivity.class, R.anim.activity_in_from_right,
                        R.anim.activity_out_from_left, "充值");
                break;
...
//                    HomeActivity.totalAssets = response.getTotalAssets(result);
                   /* if(confirmDialog == null){
                    ...
                    if (response.getStatus(result)==204) {
                        ...
                        confirmDialog.setCancelBtn("修改交易密码");
                        confirmDialog.setSureBtn("是");
                        confirmDialog.setMessage("交易密码错误，是否需要再次输入");
                        confirmDialog.show();
                    }else{
            ...
            if (bankCardArrayList.size() > 0){
                for (int i = 0; i < bankCardArrayList.size(); i++){
                    ...
                    if (bankCard.useType.trim().equals("Q")){
                        addBankCardBtn.setVisibility(View.GONE);
                        break;
                    }else if (i == bankCardArrayList.size() - 1){
                        addBankCardBtn.setVisibility(View.VISIBLE);
                    }
                }
            }
...
//        SEARCH_RECHARGE_TIMES++;
    ...
    protected void onDestroy() {
        ...
        if(InvestSureActivity.mHandler!=null){
            InvestSureActivity.mHandler.sendEmptyMessage(0);
        }
        super.onDestroy();
    }
}
